...
public class StudentLeaveDetails extends JFrame implements ActionListener{
    ...
    StudentLeaveDetails(){
        ...
        searchstudent = new Choice();
        searchstudent.setBounds(220,20, 150 , 20);
        add(searchstudent);
        try{
            Conn conn = new Conn();
            String qurey =  "Select * from tbl_student";
            ResultSet rsdata = conn.s.executeQuery(qurey);
            while(rsdata.next()){
                searchstudent.add(rsdata.getString("registrationno"));          
            }
        ...
        try{
            Conn conn = new Conn();
            String qurey =  "Select * from tbl_studentleave";
            ResultSet rsdata = conn.s.executeQuery(qurey);
    ...
    public void actionPerformed(ActionEvent actionevent){
        if(actionevent.getSource() ==  search){
            String searchquery =  "Select * from tbl_studentleave where registrationno = '"+searchstudent.getSelectedItem()+"'";
            try{
                Conn conn = new Conn();
